define(["require", "exports", "../utils/Point", "../utils/Color"], function (require, exports, Point_1, Color_1) {
    ...
    class Ellipse {
         ...
         * @author Mathieu Fehr
         */
        static drawFromBoundingRect(firstPoint, secondPoint, color, borderThickness, borderColor, image) {
            ...
            let center = new Point_1.Point((minX + maxX) / 2, (minY + maxY) / 2);
         ...
         * @author Mathieu Fehr
         */
        static drawFromCenter(center, width, height, color, borderWidth, borderHeight, borderColor, image) {
         ...
         * @author Mathieu Fehr
         */
        static drawFromEquation(center, aEllipse, bEllipse, color, aBorder, bBorder, borderColor, image) {
            // The current image
         ...
         * @author Mathieu Fehr
         */
        static getPixelColor(x, y, aEllipse, bEllipse, fillColor, aBorder, bBorder, borderColor) {
            ...
            if (Ellipse.isPixelInEllipse(x, y, aEllipse, bEllipse)) {
                ...
                if (Ellipse.isPointInEllipse(x, y, aBorder, bBorder)) {
                    let colorTemp = new Color_1.Color(fillColor.red, fillColor.green, fillColor.blue, fillColor.alpha * alphaFactor);
                    return Color_1.Color.blend(colorTemp, borderColor);
                }
                else {
                    return new Color_1.Color(fillColor.red, fillColor.green, fillColor.blue, Math.round(fillColor.alpha * alphaFactor));
                }
                ...
                for (let dx = -4; dx <= 4; dx++) {
                    for (let dy = -4; dy <= 4; dy++) {
                        if (Math.pow(((x + (dx / 8)) / aBorder), 2) + Math.pow(((y + (dy / 8)) / bBorder), 2) < 1) {
                            nbCaseFilled++;
                        }
                    }
                }
                let alphaFactor = nbCaseFilled / 81;
                return new Color_1.Color(borderColor.red, borderColor.green, borderColor.blue, Math.round(borderColor.alpha * alphaFactor));
            }
         ...
         * @author Mathieu Fehr
         */
        static isPixelInEllipse(x, y, a, b) {
            return Math.pow((x / a), 2) + Math.pow((y / b), 2)
                < Math.pow((1 - Math.sqrt(Math.pow(a, (-2)) + Math.pow(b, (-2)))), 2);
        }
        /**
         ...
         * @author Mathieu Fehr
         */
        static isPointInEllipse(x, y, a, b) {
         ...
         * @author Mathieu Fehr
         */
        static mayPixelCollidingEllipse(x, y, a, b) {
            return Math.pow((x / a), 2) + Math.pow((y / b), 2) <
                Math.pow((1 + Math.sqrt(Math.pow(a, (-2)) + Math.pow(b, (-2)))), 2);
        }
    }
