...
describe("Test of Line drawing primitives:", function() {
    ...
    // TODO change that
    let lambda = (pixel, image) => Line.paintItBlack(pixel, image, 255);
    ...
    describe("Check that the line is colored between the two points:", function() {
        ...
        it("When drawing horizontal lines", function () {
            Line.draw(imageData, new Point(5, 5), new Point(15, 5), lambda);
            for (let i = 5; i <= 15; i++) {
                assert.notEqual(imageData.data[4 * (i + 5 * 31) + 3], 0);
            }

            Line.draw(imageData, new Point(15, 15), new Point(5, 15), lambda);
            for (let i = 5; i <= 15; i++) {
        ...
        it("When drawing vertical lines", function () {
            Line.draw(imageData, new Point(5, 5), new Point(5, 15), lambda);
            for (let i = 5; i <= 15; i++) {
                assert.notEqual(imageData.data[4 * (5 + i * 31) + 3], 0);
            }

            Line.draw(imageData, new Point(15, 15), new Point(15, 5), lambda);
            for (let i = 5; i <= 15; i++) {
        ...
        it("When drawing diagonal lines", function () {
            Line.draw(imageData, new Point(0, 0), new Point(10, 10), lambda);
            for (let i = 0; i <= 10; i++) {
                assert.notEqual(imageData.data[4 * (i + 31 * i) + 3], 0);
            }

            Line.draw(imageData, new Point(30, 30), new Point(20, 20), lambda);
            for (let i = 20; i <= 30; i++) {
                assert.notEqual(imageData.data[4 * (i + 31 * i) + 3], 0);
            }

            Line.draw(imageData, new Point(0, 30), new Point(10, 20), lambda);
            for (let i = 0; i <= 10; i++) {
                let j = 30 - i;
                assert.notEqual(imageData.data[4 * (i + 31 * j) + 3], 0);
            }

            Line.draw(imageData, new Point(30, 0), new Point(20, 10), lambda);
            for (let i = 0; i <= 10; i++) {
                let j = 30 - i;
